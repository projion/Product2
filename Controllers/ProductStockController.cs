...
namespace Product2.Controllers
{
    public class ProductStockController : Controller
    {
        ...
        [HttpPost]
        public IActionResult AddUpdateStockDetails(StockDetails StockDetails, UnitInfo UnitInfo, SupplierInfo SupplierInfo, ProductInfo ProductInfo)
        {
            string url = Request.Headers["Referer"].ToString();
            //if (ModelState.IsValid)
            ...
            if (info.StockId == "False")
            {
                ...
                var temp = repoSupplierInfo.GetAll();
                var a = (SupplierInfo)temp.Where(x => x.SupplierName == SupplierInfo.SupplierName).FirstOrDefault();
                info.SupplierId = a.SupplierId;
                var b = (ProductInfo)repoProductInfo.GetAll().Where(x => x.ProductName == ProductInfo.ProductName).FirstOrDefault();
                info.ProductId = b.ProductId;
                info.Price = StockDetails.Price;
                info.Quantity = StockDetails.Quantity;
                var c = (UnitInfo)repoUnitInfo.GetAll().Where(x => x.UnitName == UnitInfo.UnitName).FirstOrDefault();
                info.Unit = c.UnitId;
                info.TotalAmount = StockDetails.TotalAmount;
            ...
            else if (info.StockId != null)
            {
                ...
                //VarVM1.SupplierInfo.SupplierId = temp.Select(x => x.SupplierId).Where();
                //VarVM1.SupplierInfo = (SupplierInfo)temp.Where(x=> x.SupplierName == VarVM1.SupplierInfo.SupplierName);
                var a = (SupplierInfo)temp.Where(x=> x.SupplierName == SupplierInfo.SupplierName).FirstOrDefault();

                //info.StockId = StockDetails.StockId;
                info.SupplierId = a.SupplierId;
                //repoProductInfo.GetAll();
                var b = (ProductInfo)repoProductInfo.GetAll().Where(x=> x.ProductName == ProductInfo.ProductName).FirstOrDefault();
                info.ProductId = b.ProductId;
                info.Price = StockDetails.Price;
                info.Quantity = StockDetails.Quantity;
